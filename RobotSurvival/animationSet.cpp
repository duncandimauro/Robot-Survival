...
AnimationSet::~AnimationSet() //properly cleans up any dynamic memory e.g. textures
{
	cleanup(spriteSheet);
...
Animation* AnimationSet::getAnimation(string p_name)
{
	for (list<Animation>::iterator a = animations.begin(); a != animations.end(); a++)
	{
...
void AnimationSet::loadAnimationSet(string fileName, list<DataGroupType>& groupTypes, bool setColorKey, int transparentPixelIndex, bool createWhiteTexture) {
	...
	if (file.good())
	{
		getline(file, imageName); //gets the name of the image at the top of the file, ex. cyborg.png
		if (setColorKey)
		{
		...
		//Now we're on the line "animations: "
		string buffer;
		getline(file, buffer);
		stringstream ss;
